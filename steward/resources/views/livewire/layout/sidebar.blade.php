...
<div class="fixed left-3 top-3 bottom-3 flex w-60 flex-col rounded-2xl bg-white dark:bg-zinc-950 border border-zinc-200 dark:border-zinc-800 shadow-sm dark:shadow-none">

    {{-- Logo --}}
    <div class="flex items-center px-5 py-4 border-b border-zinc-200 dark:border-zinc-800">
        <span class="text-lg font-semibold tracking-tight text-zinc-900 dark:text-zinc-100">StewardAI</span>
    </div>
    ...
    <div class="px-3 py-3 border-t border-zinc-200 dark:border-zinc-800 space-y-0.5">
        ...
        <div x-data class="flex items-center gap-1 rounded-lg bg-zinc-100 dark:bg-zinc-800/50 p-1 mb-2">
            <button
                ...
                :class="$store.theme.mode === 'light' ? 'bg-white dark:bg-zinc-700 shadow-sm text-zinc-900 dark:text-zinc-100' : 'text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300'"
            ...
            <button
                ...
                :class="$store.theme.mode === 'system' ? 'bg-white dark:bg-zinc-700 shadow-sm text-zinc-900 dark:text-zinc-100' : 'text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300'"
                class="flex-1 flex items-center justify-center gap-1.5 rounded-md px-2 py-1.5 text-xs font-medium transition-all"
            >
            ...
            <button
                ...
                :class="$store.theme.mode === 'dark' ? 'bg-white dark:bg-zinc-700 shadow-sm text-zinc-900 dark:text-zinc-100' : 'text-zinc-400 dark:text-zinc-500 hover:text-zinc-600 dark:hover:text-zinc-300'"
                class="flex-1 flex items-center justify-center gap-1.5 rounded-md px-2 py-1.5 text-xs font-medium transition-all"
            >
        ...
        <form method="POST" action="{{ route('logout') }}">
            ...
            >
                <x-lucide-log-out class="w-5 h-5 flex-shrink-0 text-zinc-400 dark:text-zinc-500 group-hover:text-zinc-600 dark:group-hover:text-zinc-300" />
                <span>Sign Out</span>
            </button>
        </form>
