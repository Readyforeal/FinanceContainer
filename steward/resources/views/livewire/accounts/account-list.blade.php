...
<div>
    @if ($connections->isEmpty())
        <div class="text-center py-16">
            <x-lucide-banknote class="w-12 h-12 text-zinc-400 dark:text-zinc-600 mx-auto mb-4" />
            <h2 class="text-xl font-semibold text-zinc-700 dark:text-zinc-300 mb-2">No Bank Accounts Connected</h2>
            <p class="text-zinc-400 dark:text-zinc-500 mb-6">Connect your bank account to get started.</p>
            <livewire:plaid.plaid-link />
        </div>
    @else
        <div class="flex items-center justify-between mb-6">
            <h2 class="text-lg font-semibold text-zinc-900 dark:text-zinc-100">Connected Accounts</h2>
            <livewire:plaid.plaid-link />
        </div>
        ...
        @foreach ($connections as $connection)
            <div class="mb-6">
                <div class="flex items-center gap-3 mb-3">
                    <h3 class="text-base font-medium text-zinc-800 dark:text-zinc-200">{{ $connection->institution_name }}</h3>
                    @php
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                    @foreach ($connection->accounts as $account)
                        <div class="bg-white dark:bg-zinc-800 rounded-xl p-5 border border-zinc-300 dark:border-zinc-700">
                            ...
                            <div class="text-xs text-zinc-400 dark:text-zinc-500 space-y-1">
                                ...
                                @if ($account->available_balance !== null)
                                    <div class="flex justify-between">
                                        <span>Available</span>
                                        <span class="text-zinc-500 dark:text-zinc-400">${{ number_format($account->available_balance, 2) }}</span>
                                    </div>
                                @endif
