...
<div class="fixed inset-0 lg:top-0 lg:left-[15.75rem] flex flex-col lg:flex-row lg:gap-3 lg:p-3">
    ...
    <div class="lg:hidden flex-shrink-0 relative z-10">
        <div
            class="flex items-center justify-between px-4 py-3 bg-gradient-to-b from-zinc-200/80 dark:from-zinc-950/60 to-transparent">
    ...
    @if ($showMobileHistory)
        <div class="fixed inset-0 z-50 lg:hidden">
            <div class="absolute inset-0 bg-black/30 dark:bg-black/50 backdrop-blur-sm"
                wire:click="$toggle('showMobileHistory')"></div>
            <div
                class="absolute left-0 top-0 bottom-0 w-72 bg-white dark:bg-zinc-900 border-r border-zinc-200 dark:border-zinc-800 shadow-xl flex flex-col">
                <div class="p-3 border-b border-zinc-200 dark:border-zinc-800 flex items-center justify-between">
                    ...
                    <flux:button wire:click="$toggle('showMobileHistory')" variant="ghost" size="sm"
                        icon="x" />
                </div>
                        ...
                        <div @class([
                            ...
                            <button wire:click="selectConversation({{ $conversation->id }})"
                                ...
                                <flux:text size="sm" class="mt-0.5">
                                    {{ $conversation->updated_at->diffForHumans() }}</flux:text>
                            </button>
                            <button wire:click.stop="confirmDelete({{ $conversation->id }})"
    ...
    <div
        class="hidden lg:flex w-64 flex-shrink-0 flex-col rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 overflow-hidden">
            ...
            @forelse ($conversations as $conversation)
                <div @class([
                    ...
                    <button wire:click="selectConversation({{ $conversation->id }})"
                        class="flex-1 text-left px-4 py-3 min-w-0">
                        <flux:text class="font-medium truncate">{{ $conversation->title }}</flux:text>
                        <flux:text size="sm" class="mt-0.5">{{ $conversation->updated_at->diffForHumans() }}
                        </flux:text>
                    </button>
                    <button wire:click.stop="confirmDelete({{ $conversation->id }})"
                        class="px-3 py-3 text-zinc-300 dark:text-zinc-600 hover:text-red-500 dark:hover:text-red-400 transition-colors flex-shrink-0">
    ...
    <div class="flex-1 flex flex-col relative min-h-0">
        ...
        @else
                ...
                <div class="max-w-2xl mx-auto space-y-4">
                    @forelse ($messages as $message)
                        @if ($message->role === 'user')
                            <div class="flex justify-end">
                                <div
                                    class="max-w-[85%] lg:max-w-[75%] rounded-2xl rounded-tr-sm px-4 py-2.5 bg-blue-600 text-white text-sm leading-relaxed bubble-user">
                                    {{ $message->content }}
                                </div>
                            </div>
                        @else
                            <div class="flex justify-start {{ $loop->last && !$isStreaming ? 'chat-fade-in' : '' }}">
                                <div
                                    class="max-w-[85%] lg:max-w-[75%] rounded-2xl rounded-tl-sm px-4 py-2.5 bg-white/80 dark:bg-zinc-800/80 text-zinc-900 dark:text-zinc-100 text-sm leading-relaxed bubble-assistant">
                    ...
                    @if ($isStreaming)
                        <div class="flex justify-start">
                            <div
                                class="rounded-2xl rounded-tl-sm px-4 py-3 bg-white/80 dark:bg-zinc-800/80 bubble-assistant">
                                <div class="flex items-center gap-1.5">
                                    <span
                                        class="w-2 h-2 rounded-full bg-zinc-400 dark:bg-zinc-500 animate-[thinking_1.4s_ease-in-out_infinite]"></span>
                                    <span
                                        class="w-2 h-2 rounded-full bg-zinc-400 dark:bg-zinc-500 animate-[thinking_1.4s_ease-in-out_0.2s_infinite]"></span>
                                    <span
                                        class="w-2 h-2 rounded-full bg-zinc-400 dark:bg-zinc-500 animate-[thinking_1.4s_ease-in-out_0.4s_infinite]"></span>
                                </div>
                            </div>
                        </div>
                    @endif
            ...
            <div
                class="absolute bottom-24 lg:bottom-0 left-0 right-0 px-3 lg:px-8 pb-2 lg:pb-4 pt-8 pointer-events-none">
        ...
        <div class="fixed inset-0 z-[100] flex items-center justify-center" wire:keydown.escape="cancelDelete">
            ...
            <div
                ...
                <div class="flex items-start gap-4">
                    <div
                        class="flex items-center justify-center w-10 h-10 rounded-full bg-red-100 dark:bg-red-900/30 flex-shrink-0">
                        <flux:icon.trash-2 class="size-5 text-red-600 dark:text-red-400" />
                    </div>
                    <div>
                        <flux:heading size="sm">Delete conversation</flux:heading>
                        <flux:text size="sm" class="mt-1">This will permanently delete this conversation and all
                            its messages.</flux:text>
                    </div>
                </div>
