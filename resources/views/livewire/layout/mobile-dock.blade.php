...
<div x-data="{ expanded: false, path: window.location.pathname }"
    ...
    <div x-show="expanded" x-cloak x-transition:enter="transition-all duration-500 ease-[cubic-bezier(0.16,1,0.3,1)]"
        ...
        class="fixed bottom-20 left-3 z-50 rounded-3xl bg-white/50 dark:bg-zinc-900/50 backdrop-blur-xl border border-zinc-200/50 dark:border-zinc-700/50 shadow-xl overflow-hidden"
        :class="$store.dockAction?.icon ? 'right-16' : 'right-3'">
    ...
    <div class="fixed bottom-3 left-3 right-3 z-40 flex items-center gap-2">
        {{-- Dock bar --}}
        <div class="flex-1 flex items-center justify-around bg-white/50 dark:bg-zinc-900/50 backdrop-blur-xl rounded-3xl shadow-lg shadow-zinc-300/30 dark:shadow-zinc-950/40 border border-zinc-200/50 dark:border-zinc-700/50 px-2 py-2.5 transition-all duration-300 ease-[cubic-bezier(0.16,1,0.3,1)]">
        ...
        <div x-data="{ show: false, icon: null }"
             x-init="
                $watch('$store.dockAction', (val) => {
                    if (val?.icon) { icon = val.icon; show = true; }
                    else { show = false; }
                });
            ...
            <button
                ...
                class="flex items-center justify-center size-[46px] rounded-3xl bg-accent/80 text-accent-foreground backdrop-blur-xl shadow-lg">
                <template x-if="icon === 'plus'"><flux:icon.plus variant="mini" /></template>
            </button>
        </div>
    </div>
</div>
